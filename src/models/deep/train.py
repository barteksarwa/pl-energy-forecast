"""Training loop for the LSTM variants. Pinball loss on 3 quantiles.

Discipline (ml-training-ops): config printed first, timestamped epoch lines,
per-run checkpoints, early stopping on validation pinball, fixed seeds.
"""
...
def train_variant(
    ...
    tr = _loader(train, batch, shuffle=True)  # shuffling SAMPLES (whole days) is safe
