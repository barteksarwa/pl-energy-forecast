...
def _pl_holidays(years: list[int]) -> set[dt.date]:
    return set(holidays_pkg.country_holidays("PL", years=years).keys())


def bridge_days(years: list[int]) -> set[dt.date]:
    """Friday after a Thursday holiday, Monday before a Tuesday holiday."""
    hols = _pl_holidays(years)
...
def calendar_features(hours_utc: pd.DatetimeIndex, tz: str = LOCAL_TZ) -> pd.DataFrame:
